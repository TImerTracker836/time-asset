...
export function TodayTimeline({ date }: { date: string }) {
  const { getEntriesByDate, getCategoryById, getPlanByDate, getUnrecordedGaps } = useAppStore()
    ...
    <div className="relative overflow-y-auto" style={{ height: '100%' }}>
      <div className="relative" style={{ height: `${24 * HOUR_HEIGHT}px` }}>
        {hours.map((h) => (
          ...
          >
            <span className="text-xs text-text-muted w-10 text-right pr-2 select-none -mt-2">
          ...
          return (
            <div
              ...
              style={{
                top: `${top}px`,
                height: `${height}px`,
                backgroundColor: `${cat?.color || 'var(--accent)'}08`,
                borderLeft: `2px dashed ${cat?.color || 'var(--accent)'}66`,
              }}
            >
              <div className={`flex items-center gap-1 px-2 py-1 h-full ${block.isCompleted ? 'opacity-40' : ''}`}>
                <span className="text-sm">{cat?.icon}</span>
                <span className="text-xs text-text-secondary truncate">{block.title}</span>
                <span className="text-xs text-text-muted shrink-0">{block.startTime}-{block.endTime}</span>
                {block.isCompleted && <Check size={12} className="text-success shrink-0 ml-auto" />}
              </div>
            </div>
          )
        })}
        ...
        {gaps.map((gap, i) => {
          ...
          return (
            ...
            >
              <AlertTriangle size={12} className="text-danger/40 shrink-0" />
              <span className="text-xs text-danger/50">
                {gap.minutes >= 60
          ...
          return (
            <div
              ...
              style={{
                top: `${top}px`,
                height: `${height}px`,
                backgroundColor: typeConfig?.bg || 'var(--color-invest-bg)',
            ...
            >
              <div className="flex items-center gap-1 overflow-hidden">
                ...
                {height > 35 && (
                  <span className="text-xs text-text-muted ml-auto shrink-0">{entry.duration}m</span>
                )}
              </div>
              {height > 48 && (
        ...
        {date === format(new Date(), 'yyyy-MM-dd') && (() => {
          ...
          return (
            <div className="absolute left-10 right-0 flex items-center pointer-events-none" style={{ top: `${y}px` }}>
              <div className="w-2 h-2 rounded-full bg-danger shrink-0" />
              <div className="flex-1 border-t border-danger border-dashed" />
            </div>
          )
        })()}
      </div>
